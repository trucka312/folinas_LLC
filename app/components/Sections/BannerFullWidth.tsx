...
const BannerFullWidth = ({className, title, image, imageClassName, imageWidth, imageHeight, imageAlt, content, contentClassName}: BannerFullWidthProps) => {    
    return (
        <div className={`relative ${className&&className}`}>
            ...
            <div className={`absolute top-[50%] -translate-y-[50%] left-0 right-0 z-1 px-[50px] py-10 text-white ${fontSecondary.className}`}>
                {title && <Heading headingTag="h2" heading={title} className="uppercase text-[20px] !mb-1 animate-fadeInDown" />}
                <div className={`text-[30px] max-w-[700px] leading-9 ${contentClassName&&contentClassName}`} dangerouslySetInnerHTML={{__html: content}}></div>
            </div>
        </div>
    )
}
